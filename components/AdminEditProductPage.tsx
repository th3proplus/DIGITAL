...
export const AdminEditProductPage: React.FC<AdminEditProductPageProps> = ({ productToEdit, onSave, onCancel, categories }) => {
    const { translations } = useI18n();
    const { settings } = useSettings();
    const [productData, setProductData] = useState<Product>(productToEdit);
                            ...
                            {productData.variants.map((variant, index) => (
                                <div key={variant.id} className="grid grid-cols-12 gap-3 p-4 bg-gray-50 rounded-lg border">
                                    <div className="col-span-12 sm:col-span-5">
                                        ...
                                        <input value={variant.nameKey} onChange={e => handleVariantChange(index, 'nameKey', e.target.value)} className="w-full mt-1 border-gray-300 rounded-md shadow-sm text-sm p-2" />
                                    </div>
