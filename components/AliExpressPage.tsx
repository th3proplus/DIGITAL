...
export const AliExpressPage: React.FC<AliExpressPageProps> = ({ onAddToCart, onBackToStore }) => {
    ...
    const validate = () => {
        ...
        if (!url.startsWith('https://') || !url.includes('aliexpress.com')) {
            newErrors.url = t('aliexpress_shopper.error_url');
        }
        if (price === '' || price <= 0) {
            newErrors.price = t('aliexpress_shopper.error_price');
        }
        if (shippingPrice === '' || shippingPrice < 0) {
            newErrors.shippingPrice = t('aliexpress_shopper.error_shipping_price');
        }
        setErrors(newErrors);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newItem: CartItem = {
            productId: 'aliexpress-shopper',
            variantId: `custom-${Date.now()}`,
            quantity: 1, 
            productNameKey: productName || 'AliExpress Item', 
            variantNameKey: t('aliexpress_shopper.custom_order_variant'),
            logoUrl: 'https://i.imgur.com/24H2s2t.png',
            price: finalPriceInStoreCurrency,
            isFreeTrial: false,
            metadata: {
                isCustomOrder: true,
                customOrderType: 'aliexpress',
                url: url,
            ...
            <div className="container py-16">
                ...
                <section className="text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                       <HowItWorksStep icon="link" title={t('aliexpress_shopper.step1_title')} description={t('aliexpress_shopper.step1_desc')} />
                       <HowItWorksStep icon="wallet" title={t('aliexpress_shopper.step2_title')} description={t('aliexpress_shopper.step2_desc')} />
                       <HowItWorksStep icon="package" title={t('aliexpress_shopper.step3_title')} description={t('aliexpress_shopper.step3_desc')} />
                    </div>
                </section>
                    ...
                    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-start">
                            ...
                            <div className="lg:col-span-3">
                                <h2 className="text-2xl font-bold text-brand-text-primary mb-6">{t('aliexpress_shopper.form_title')}</h2>
                                <div className="space-y-5">
                                    <InputField 
                                        label={t('aliexpress_shopper.form_url')} 
                                        id="url" 
                                        type="url" 
                                        value={url} 
                                        onChange={(e) => setUrl(e.target.value)} 
                                        required 
                                        placeholder={t('aliexpress_shopper.form_url_placeholder')}
                                        error={errors.url}
                                    />
                                    <InputField 
                                        label={t('aliexpress_shopper.form_product_name')} 
                                        id="productName" 
                                        type="text" 
                                        value={productName} 
                                        onChange={(e) => setProductName(e.target.value)} 
                                        required 
                                        placeholder={t('aliexpress_shopper.form_product_name_placeholder')}
                                    />
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <InputField
                                            label={t('aliexpress_shopper.form_price_per_item')}
                                            id="price"
                                            type="number"
                                            value={price}
                                        ...
                                        />
                                        <InputField
                                            label={t('aliexpress_shopper.form_shipping_price')}
                                            id="shipping_price"
                                            type="number"
                                            value={shippingPrice}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            <label htmlFor="currency" className="block text-sm font-medium text-gray-700 mb-1.5">{t('aliexpress_shopper.form_currency')}</label>
                                            <select
                                                id="currency"
                                                value={currency}
                                        ...
                                        <div>
                                            <label htmlFor="quantity" className="block text-sm font-medium text-gray-700 mb-1.5">{t('aliexpress_shopper.form_quantity')}</label>
                                            <input
                                                id="quantity"
                                                type="number"
                                                value={quantity}
                            ...
                            <div className="lg:col-span-2 lg:sticky top-24">
                                <h3 className="text-lg font-bold text-brand-text-primary mb-4">{t('aliexpress_shopper.price_breakdown')}</h3>
                                {priceCalculation ? (
                                    <div className="space-y-3">
                                        <div className="flex justify-between text-brand-text-secondary text-sm">
                                            <span>{t('aliexpress_shopper.original_price')}</span>
                                            <span className="font-medium">{priceCalculation.originalProductCostFormatted}</span>
                                        </div>
                                        <div className="flex justify-between text-brand-text-secondary text-sm">
                                            <span>{t('aliexpress_shopper.shipping_cost')}</span>
                                        ...
                                        <div className="flex justify-between items-center font-bold text-2xl text-brand-text-primary">
                                            <span>{t('aliexpress_shopper.total_price')}</span>
                                ...
                                >
                                    ...
                                    {t('aliexpress_shopper.add_to_cart_btn')}
                                </button>
                            </div>
                        </div>
                    </form>
